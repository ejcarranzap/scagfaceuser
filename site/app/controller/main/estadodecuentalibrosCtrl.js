(function () {
	...
	app.controller('estadodecuentalibrosCtrl', function ($scope, $log, $filter, boService, $uibModal,
  userService) {
		$scope.data = [];
		$scope.record = {};
      ...
      console.log($scope.alumnos);
		...
		$scope.open = function (record, size, actn) {
			var modalInstance = $uibModal.open({
				...
				templateUrl: 'app/view/main/estadodecuentalibrosdet.html?' + (new Date()).getTime(),
				controller: 'estadodecuentalibrosdetCtrl',
				size: size,
		...
		$log.info('estadodecuentalibrosCtrl');
	});

	app.controller('estadodecuentalibrosdetCtrl', function ($scope, $log, $filter, $window, boService, fileServicePHP, $uibModalInstance,
		record) {
		$scope.record = record;
		...
		$log.info('estadodecuentalibrosdetCtrl');
	});
}());
