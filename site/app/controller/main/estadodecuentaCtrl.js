(function () {
	...
	app.controller('estadodecuentaCtrl', function ($scope, $log, $filter, boService, $uibModal,
  userService) {
		$scope.data = [];
		$scope.record = {};
      ...
      console.log($scope.alumnos);
		...
		$scope.open = function (record, size, actn) {
			var modalInstance = $uibModal.open({
				...
				templateUrl: 'app/view/main/estadodecuentadet.html?' + (new Date()).getTime(),
				controller: 'estadodecuentadetCtrl',
				size: size,
		...
		$log.info('estadodecuentaCtrl');
	});

	app.controller('estadodecuentadetCtrl', function ($scope, $log, $filter, $window, boService, fileServicePHP, $uibModalInstance,
		record) {
		$scope.record = record;
		...
		$log.info('estadodecuentadetCtrl');
	});
}());
